				...
				<form action='{{  Request::url() }}' method='GET'>
					...
					<div>
						<div class="col-12 text-right">
							...
							<a  target='_blank' href = "{{route('exportBonus')}}?username={{ request('username') }}&bonus={{$bonus}}&from={{ request('from') }}&to={{ request('to') }}"><button  type='button' class="btn btn-gradient-primary btn-sm px-3">{{ __('site.EXPORT') }}</button></a>
						</div>
					</div>
				</form>
			</div>
	...
	<div class="col-12">
		<div class="card">
			<div class="card-body">
				...
				<h4 class="mt-0 header-title">{{ __('site.Total') }}:{{$total}}</h4>
